...
pool.query("ALTER TABLE flight_documents ADD COLUMN IF NOT EXISTS activity_id UUID REFERENCES day_activities(id) ON DELETE CASCADE").catch(e => console.error('[startup] activity_id migration:', e.message));
...
pool.query(`CREATE TABLE IF NOT EXISTS known_airlines (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), airline_name TEXT UNIQUE NOT NULL, notification_domain TEXT, has_dedicated_parser BOOLEAN DEFAULT false, first_seen TIMESTAMPTZ DEFAULT NOW())`).catch(e => console.error('[startup] known_airlines:', e.message));
pool.query(`CREATE TABLE IF NOT EXISTS flight_change_alerts (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), flight_id UUID REFERENCES flights(id) ON DELETE CASCADE, trip_id UUID REFERENCES trips(id) ON DELETE CASCADE, source_email_id TEXT UNIQUE, change_type VARCHAR(30), old_values JSONB, new_values JSONB, status VARCHAR(20) DEFAULT 'pending', raw_email_snippet TEXT, detected_at TIMESTAMPTZ DEFAULT NOW(), resolved_at TIMESTAMPTZ)`).catch(e => console.error('[startup] flight_change_alerts:', e.message));
pool.query(`CREATE TABLE IF NOT EXISTS flight_documents (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), flight_id UUID REFERENCES flights(id) ON DELETE CASCADE, trip_id UUID REFERENCES trips(id) ON DELETE CASCADE, traveler_name TEXT, document_type VARCHAR(20) DEFAULT 'boarding_pass', filename TEXT NOT NULL, mime_type TEXT DEFAULT 'application/pdf', file_data BYTEA NOT NULL, source_email_id TEXT, uploaded_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(source_email_id, filename))`).catch(e => console.error('[startup] flight_documents:', e.message));
pool.query(`INSERT INTO known_airlines (airline_name) SELECT DISTINCT airline FROM flights WHERE airline IS NOT NULL AND airline != '' ON CONFLICT (airline_name) DO NOTHING`).catch(e => console.error('[startup] known_airlines backfill:', e.message));
